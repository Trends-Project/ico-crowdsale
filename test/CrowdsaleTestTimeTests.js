...
contract('Crowdsale', accounts => {
    ...
    it('during private sale, funds can be withdrawn', async function () {
        ...
        await this.crowdsale.setIcoDates(
            date - 5*day, // pre ico
            date - 4*day, // 2nd round
            date + 1*day, // end of pre ico 
            date + 2*day,   // main ico
            date + 3*day,   // end of main
        );
        ...
        await this.crowdsale.setIcoDates(
            date + 1*day, // pre ico
            date + 2*day, // 2nd round
            date + 3*day, // end of pre ico
            date + 4*day,   // main ico
            date + 5*day,   // end of main
        );        
        // can't buy before pre ico
        await this.crowdsale.procureTokens(account, { from: account, value: 11*ETHER, gas: 500000 }).should.be.rejectedWith(EVMRevert);
        await this.crowdsale.transferEthToMultisig().should.be.fulfilled;
        ...
        await this.crowdsale.procureTokens(account, { from: account, value: 11*ETHER, gas: 500000 }).should.be.fulfilled;
        await this.crowdsale.transferEthToMultisig().should.be.fulfilled;
        
        // === during 2nd round
        await this.crowdsale.setIcoDates(
            date - 2*day, // pre ico
            date - 1*day, // 2nd round
            date + 3*day, // end of pre ico
            date + 4*day,   // main ico
            date + 5*day,   // end of main
        );        
